...
int32_t main(void)
{
    ...
    stc_gpio_config_t       stcPCA1Port;
    stc_gpio_config_t       stcPOutPort;
    uint16_t                u16InitCntData = 0;
    
    DDL_ZERO_STRUCT(stcConfig);
    DDL_ZERO_STRUCT(stcModConfig);
    DDL_ZERO_STRUCT(stcPCA1Port);
    DDL_ZERO_STRUCT(stcPOutPort);
    ...
    stcPCA1Port.enDrv  = GpioDrvH;
    stcPCA1Port.enDir  = GpioDirIn;
    Gpio_Init(GpioPortA, GpioPin7, &stcPCA1Port);
    Gpio_SetAfMode(GpioPortA,GpioPin7,GpioAf2);
    
    //PA06设置为输出信号脚
    //Gpio_ClrAnalogMode(GpioPortA, GpioPin6);
    stcPOutPort.enDrv  = GpioDrvH;
    stcPOutPort.enDir  = GpioDirOut;
    Gpio_Init(GpioPortA, GpioPin6, &stcPOutPort);
    Gpio_SetAfMode(GpioPortA,GpioPin6,GpioAf0);
    
    Gpio_WriteOutputIO(GpioPortA, GpioPin6, TRUE);     //PA6输出高

    
    stcConfig.enCIDL = IdleGoOn;                 //休眠模式PCA工作 
    stcConfig.enWDTE = PCAWDTDisable;            //wdt功能关闭
    stcConfig.enCPS  = PCAPCLKDiv8;              //PCLK/8,
    ...
    delay1ms(2000);
    Gpio_WriteOutputIO(GpioPortA, GpioPin6, FALSE); //内部输出低，外接PA06产生下降沿中断
    ...
    delay1ms(2000);
    Gpio_WriteOutputIO(GpioPortA, GpioPin6, TRUE);  //内部输出高，外接PA06产生上升沿中断
    //进入中断……
    while(1)
    {
        //发生上升沿捕获
