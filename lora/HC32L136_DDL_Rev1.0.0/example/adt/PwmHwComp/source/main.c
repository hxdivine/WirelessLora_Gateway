...
void Adt4UnderFullCalllback(void)
{
    ...
    if (i%2)
    {
        Adt_SetCompareValue(AdtTIM4, AdtCompareC, 0xBBBB);
    }
    else
    {
        Adt_SetCompareValue(AdtTIM4, AdtCompareC, 0x8888);
    }
    i++;
}
...
int32_t main(void)
{
    ...
    u16Period = 0xEEEE;
    Adt_SetPeriod(enAdt, u16Period);                         //周期值
    ...
    Adt_SetDTUA(enAdt, 0x2000);
    Adt_SetDTDA(enAdt, 0x2000);
